...
short yRawTouch[DRV_TOUCH_ADC_SAMPLE_POINTS] = { ${Val_CAL_LRX_SAMPLE},
                                                 ${Val_CAL_LRY_SAMPLE},
                                                 ${Val_CAL_LLX_SAMPLE},
                                                 ${Val_CAL_LLY_SAMPLE} };
...
short DRV_Touch_ADC_PositionDetect(void)
{
    ...
    switch ( pDrvInstance->state ) 
    {
        ...
        {            
            if ( !ADCHS_ChannelResultIsReady(${DRV_TOUCH_AN_INPUT_Y}) )
            {
                break; 
            }
            if ( (uint16_t) ${CONFIG_DRV_TOUCH_ADC_PRESS_THRESHOLD} <
                (uint16_t) ADCHS_ChannelResultGet(${DRV_TOUCH_AN_INPUT_Y})>>2 )
            {
                adcX = -1;
                adcY = -1;
            } 
        ...
        {
            if ( !ADCHS_ChannelResultIsReady(${DRV_TOUCH_AN_INPUT_X}) )
            {
                break;
            }

            if ((uint16_t) ${CONFIG_DRV_TOUCH_ADC_PRESS_THRESHOLD} >
                (uint16_t) ADCHS_ChannelResultGet(${DRV_TOUCH_AN_INPUT_X})>>2)
            {
                //y+
        ...
        case DRV_TOUCH_ADC_STATE_GET_X:
        {
            ...
            temp = (uint16_t) ADCHS_ChannelResultGet(${DRV_TOUCH_AN_INPUT_X})>>2;
            if (temp != tempX) 
            {
                tempX = temp;
                pDrvInstance->state = DRV_TOUCH_ADC_STATE_RUN_X;
        ...
        case DRV_TOUCH_ADC_STATE_GET_Y:
        {
            ...
            temp = (uint16_t) ADCHS_ChannelResultIsReady(${DRV_TOUCH_AN_INPUT_Y})>>2;
...
void DRV_Touch_ADC_TouchDataRead(void)
{
    DRV_TOUCH_ADC_OBJECT * pDrvInstance = (DRV_TOUCH_ADC_OBJECT *)&sADCDriverInstances;
    pDrvInstance->touchStatus = DRV_TOUCH_POSITION_NONE;
}
